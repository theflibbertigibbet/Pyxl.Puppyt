...
export function usePlayback({ numFrames, playbackSpeed = 1 }: PlaybackOptions) {
    ...
    // FIX: The `animate` function must accept a `time` parameter from `requestAnimationFrame` for calculating animation delta time.
    ...
    useEffect(() => {
        if (isPlaying) {
            ...
            // Reset lastTimeRef on pause to prevent a large time jump when resuming playback.
