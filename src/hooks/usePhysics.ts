...
export function usePhysics({ targetPose, isEnabled }: PhysicsOptions): PoseData {
    ...
    // FIX: The `animate` function must accept a `time` parameter from `requestAnimationFrame` for physics simulation delta time.
    const animate = useCallback((time: number) => {
        ...
        // Safely calculate time delta by providing a fallback for the first frame, preventing NaN errors.
        ...
        if (dt > 0) {
          ...
          // Use a functional state update for `setPhysicalPose` to remove an unnecessary dependency and prevent re-renders.
    ...
    useEffect(() => {
        if (isEnabled) {
            ...
            physicsBodyRef.current = createPhysicsBodyFromPose(targetPose);
            // Reset lastTimeRef when physics is disabled to prevent a large time jump on re-enable.
