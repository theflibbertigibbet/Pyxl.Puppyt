...
export const Canvas = forwardRef<CanvasHandle, CanvasProps>(({ pose, onPoseCommit, prevPose, nextPose, assets, selectedPartKey, onSelectPart, onDeselect }, ref) => {
  ...
  useEffect(() => {
    // FIX: The `animate` function must accept a `time` parameter to be compatible with `requestAnimationFrame`.
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    if (e.shiftKey && finalClickedKey && !['torso', 'waist', 'head', 'neck', 'ground'].includes(finalClickedKey)) {
        onSelectPart(finalClickedKey);
        setDraggedPartKey(finalClickedKey);
        dragModeRef.current = 'aim';

        // The controlled joint is the one associated with the clicked part (e.g., 'left.elbow').
        // Its pivot point is the joint itself.
  ...
  const handleRotate = (skeleton: Skeleton, snappedPos: Point, useSnap: boolean, bypassConstraints: boolean) => {
    ...
    if (!bypassConstraints) {
        const constraint = jointConstraints[jointToRotate as keyof typeof jointConstraints];
        if (constraint) {
  ...
  const handleAim = (pos: Point) => {
    ...
    // 1. Calculate the absolute angle of the gesture on the grid.
    const targetGridAngle = Math.atan2(pos.y - activePivot.y, pos.x - activePivot.x);

    // 2. Get the parent's current world rotation.
    const parentGlobalRotation = getParentWorldAngle(controlledJoint, localPose);
    if (parentGlobalRotation === null) return;

    // 3. The local angle is the target grid angle minus the parent's rotation.
    ...
    setLocalPose(p => {
        ...
        if (parts.length === 2) {
            ...
            // This is how PoseData is structured
            (newPose[side] as any)[jointName] = finalLocalAngle;
        } else {
            // This logic is for non-side-specific parts like 'torso', 'waist', 'head'
            // which are excluded from aim mode anyway.
